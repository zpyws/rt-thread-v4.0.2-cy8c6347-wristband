...
static struct udcd _cypress_udc;
...
int cy8c63_usbd_register(void)
{
    rt_memset((void *)&_cypress_udc, 0, sizeof(struct udcd));
    _cypress_udc.parent.type = RT_Device_Class_USBDevice;
#ifdef RT_USING_DEVICE_OPS
    _cypress_udc.parent.ops = &_ops;
#else
    _cypress_udc.parent.init = _init;
#endif
//    _cypress_udc.parent.user_data = &_stm_pcd;
    _cypress_udc.ops = &_udc_ops;
    /* Register endpoint infomation */
    _cypress_udc.ep_pool = _ep_pool;
    _cypress_udc.ep0.id = &_ep_pool[0];
#ifdef BSP_USBD_SPEED_HS
    _cypress_udc.device_is_hs = RT_TRUE;
#endif
    rt_device_register((rt_device_t)&_cypress_udc, "usbd", 0);
    rt_usb_device_init();
    return RT_EOK;
}
INIT_DEVICE_EXPORT(cy8c63_usbd_register);
#endif
